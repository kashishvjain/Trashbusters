...
def main():
    ...
    # Kalman Filter
    tracker = Tracker(
        dist_thresh=160, max_frames_to_skip=100, max_trace_length=5, trackIdCount=1
    )
    ...
    while True:
        ...
        for output_kalman_normal in outputs_kalman_normal:
            cv2.rectangle(
                orig_im,
                (int(output_kalman_normal[0]), int(output_kalman_normal[1])),
                (int(output_kalman_normal[2]), int(output_kalman_normal[3])),
                (255, 255, 255),
                2,
            )
            cv2.putText(
                orig_im,
                str(output_kalman_normal[4]),
                (int(output_kalman_normal[0]), int(output_kalman_normal[1])),
                0,
                5e-3 * 200,
                (0, 255, 0),
            ...
            for output_kalman_normal in outputs_kalman_normal:
                if output_kalman_normal[4] == 1:
                    cv2.putText(
                        ...
                        person,
                        (
                            output_kalman_normal[0].astype(np.int32) + 100,
                            output_kalman_normal[1].astype(np.int32) + 20,
                        ),
                        cv2.FONT_HERSHEY_PLAIN,
                        2,
                        [0, 255, 0],
