...
def main():
    ...
    model_facenet.load_state_dict(
        torch.load(saved_model, map_location=torch.device("cpu"))["backbone_net_list"]
    )
    model_facenet.eval()
    ...
    while True:
        ...
        output[:, 1:5] = (
            torch.clamp(output[:, 1:5], 0.0, float(inp_dim)) / inp_dim
        )  # 夹紧张量，限制在一个区间内
        ...
        for person_bbox in output:
            ...
            if len(bboxes) == 0:
                ...
                for bbox in bboxes:
                    ...
                    feature = np.squeeze(
                        get_feature(face_img, model_facenet, trans, device)
                    )  # 框里的图像计算feature
