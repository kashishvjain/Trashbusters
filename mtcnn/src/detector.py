...
def detect_faces(
    ...
    if bounding_boxes:  # add a condition
        ...
        keep = nms(
            bounding_boxes[:, 0:5], nms_thresholds[0]
        ...
        )  # 用reg*系列（对应坐标的线性回归参数）可对box进行坐标修正
        # shape [n_boxes, 5]

        bounding_boxes = convert_to_square(
            bounding_boxes
        )  # 目标框修正之后，先rec2square、再pad。rec2square是将修正后不规则的框调整为正方形，pad的目标是将超出原img范围的部分填充为0，大小比例不变。
        ...
        img_boxes = get_image_boxes(
            bounding_boxes, image, size=24
        )  # 将P-Net最后输出的所有box，resize到24*24后输入R-Net。
        ...
        """
        img_boxes = get_image_boxes(
            bounding_boxes, image, size=48
        )  # 将R-Net最后输出的所有box，resize到48*48后输入O-Net。
        if len(img_boxes) == 0:
            return [], []
        img_boxes = Variable(torch.FloatTensor(img_boxes), volatile=True)
        if torch.cuda.is_available():
        ...
        landmarks = output[
            0
        ].data.numpy()  # shape [n_boxes, 10]            #比前面多了一个输出landmarks脸部的关键点位置，每张脸五个点
        ...
        keep = nms(
            bounding_boxes, nms_thresholds[2], mode="min"
        )  # Non-maximum suppression.
        bounding_boxes = bounding_boxes[keep]
        landmarks = landmarks[keep]
